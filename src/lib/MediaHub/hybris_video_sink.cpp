...
public:
        ...
        gl_texture_consumer{gl_consumer_create_by_id_with_igbc(gl_texture, graphics_buffer_consumer)}
    {
        if (not graphics_buffer_consumer) {
            MH_ERROR("video::HybrisGlSink: Could not connect to remote buffer queue.");
            return;
        };

        if (not gl_texture_consumer) {
            MH_ERROR("video::HybrisGlSink: Could not associate local texture id with remote buffer streak.");
            return;
        };
